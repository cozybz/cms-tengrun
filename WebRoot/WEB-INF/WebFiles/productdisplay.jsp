   ...
   <div id="pageB" style="height:300px; margin-top:3px;float: left">
    ...
    <div id="pageC"  style="border:1px solid #099;margin-top:3px;margin-left:5px;width: 723px;height:566px;background:u#f6f7f8;float: left;">
	       ...
	       <div style="width: 723px;height: 30px;background-image:url(images/zhuti.jpg); " >
	      			<div style="font-weight:bold; margin-left: 22px;padding-top: 5px;"><p>${all.get(0).getProductType()}</p></div>
		    </div>
	  		  <div style="height: 485px;">
	    
				<c:if test="${all.get(0).getProductType()!='新闻'}">	
<c:forEach var="product" items="${requestScope.all}">
			    							<div class="imgbox">
			    							<div class="imgbox_img"><a href="InfoDisplay?id=${product.getId()}"><img width="220px" height="180px" src="${product.getImgUrl()}" /></a></div>
			    							<div class="imgbox_name"><center><a href="InfoDisplay?id=${product.getId()}"><p>${product.getName()}</p></a></center></div>
			    							</div>
</c:forEach>
			    	</c:if>				
	    ...
	    <c:forEach var="product" items="${requestScope.all}">
	    <li><a href="InfoDisplay?id=${product.getId()}">${product.getName()}</a></li>
	    </c:forEach>
	    </ul>
	    </c:if>			
	    		</div>
	    		...
	    		<div style="margin-top:15px;;width:723px;border-top:1px solid #099"><p style="margin-left: 470px;"> <c:if test="${requestScope.curpage>1}"> 
							<a href="InfoDisplay?curpage=1">首页</a>
							<a href="InfoDisplay?curpage=${requestScope.curpage-1 }">上一页</a>
							</c:if>
							...
							<a href="InfoDisplay?curpage=${requestScope.curpage+1 }">下一页</a>
							<a href="InfoDisplay?curpage=${requestScope.totalpage}">末页</a>
							</c:if>
							<c:if test="${requestScope.curpage>=requestScope.totalpage}">
							下一页
							末页
