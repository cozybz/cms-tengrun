  ...
  <div style="width:960px; background: #999; margin:0 auto;">
        ...
        <div style=" width:750px; height:450px; background-color:#F2F2F2; margin-left:5px; float:left;">
        	<div style=" background: url(images/zhuti.jpg); width:750px; height:30px;">
            	...
            	<div style="width:90px; height:30px; margin-left:35px; font-weight:bold; padding-top:10px;">修改产品</div>
            	</div>
            	<div>
            	
            	<form action="admin/UpdateProduct_do" method="post" onsubmit="return test()">
            	<input type="hidden" name="id" value="${requestScope.product.getId()}">
<table>
<tr>
<td colspan="2">产品名称：<input type="text" name="name" value="${requestScope.product.getName()}"></td>
</tr>
<tr>
<td colspan="2">图片地址：<input type="text" name="imgUrl" value="${requestScope.product.getImgUrl() }"></td>
</tr>
<tr>
<td colspan="2">产品类别：
	...
	<c:choose>
			<c:when test="${productType.getProductType()==requestScope.product.getProductType()}">
				<option value="${productType.getProductType()}" selected="selected">${productType.getProductType()} </option>
			</c:when>
			<c:otherwise>
...
<tr>
<td colspan="2"> <textarea class="ckeditor"  name="information">${requestScope.product.getInformation() }</textarea>  </td>
</tr>
